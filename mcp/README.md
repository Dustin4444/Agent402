...
- When a call hits HTTP 402: with a wallet key set (`AGENT_KEY` for the EVM chains — Base/Polygon/Arbitrum/Monad/Celo/Avalanche, `SOLANA_AGENT_KEY` for Solana), the server signs an x402 USDC payment on a chain the seller accepts and retries; without a key it solves the tool's proof-of-work challenge (~0.2 s of CPU) on the eligible tools. (The service also settles USDC on Celo, Avalanche, Stellar and Algorand, and USDG on Robinhood Chain — 10 chains total — for callers using a raw x402 client rather than this package.)
