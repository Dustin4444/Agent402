...
Expected: rewrites the static surfaces to 1,416; `--check` exits 0.
...
Expected: decode tests pass; test-all reports the two new routes as exercised (lenient/NETWORK), zero strict failures, total tool count 1,416.

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "B20 fast-follows: CI decode-test wiring + tool count 1,416 across static surfaces"
```
...
Expected: count 1,416; find resolves b20-memos.
