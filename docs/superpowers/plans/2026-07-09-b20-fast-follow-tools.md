...
- After adding tools: run `node scripts/sync-count.js` (1,417 → 1,417).
...
Expected: rewrites the static surfaces to 1,417; `--check` exits 0.
...
Expected: decode tests pass; test-all reports the two new routes as exercised (lenient/NETWORK), zero strict failures, total tool count 1,417.

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "B20 fast-follows: CI decode-test wiring + tool count 1,417 across static surfaces"
```
...
Expected: count 1,417; find resolves b20-memos.
