# The 501 — Phase 4 (Brand Sweep) + Phase 5 (Launch v2.0.0) Plan

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:subagent-driven-development. Steps use checkbox syntax.

**Goal:** Flip the entire external footprint from "1,432 tools" to "**501 tools · 100 skill packs**, CI-capped" — website, SEO, GitHub, every registry, and all images — then ship the single atomic **v2.0.0** launch deploy with on-chain proof of every new tool. Spec: `docs/superpowers/specs/2026-07-13-the-501-overhaul-design.md` (Phases 4-5), whose Phase 4 sub-inventory (4a-4e) is the requirements source.
...
- **The launch narrative** (owner-approved framing): "Only 501. 400 tools + 100 skill packs. Every one tested, priced, and settled on-chain. Nothing else made the cut. Paid in USDC on 8 chains — or free via proof-of-work compute on the pure-CPU ones." Final hero/tweet copy = owner-approved before it ships.
...
- Nothing claims "501" on a live surface until the deploy that makes it true — the sweep/publish/register steps run AFTER or WITH the deploy, never before.
...
- [ ] Grep every hardcoded count-narrative string that isn't the bare "N tools" number sync-count owns: `grep -rn "tools\|skill pack\|converter\|deterministic" src/ledger-home.js src/market-page.js src/pages.js | grep -iE "501|400|100|whole|every"`. Update the FRAMING to the curation story ("Only 501. Every one earns its place." + "400 tools + 100 skill packs"). Keep the neutral-index line.
...
- [ ] Boot smoke: homepage renders "501"/"400"/"100" correctly; all suites green. Commit `brand(1): website curation-story copy`.
...
- [ ] Note (DEPLOY-TIME): `scripts/indexnow-submit.js` ping fires AFTER the deploy (Phase 5) so Bing/Copilot/DDG re-crawl the live 501 — add it to the Phase-5 checklist, do not run now.
- [ ] Suites green. Commit `brand(2): SEO/JSON-LD/llms.txt reflect 501` (if any non-derived change; else skip with a note).
...
- [ ] **X header**: redo the dark 8-chain header at "501 · 100" (base the renderer on the existing header renderer — grep `.remember/tmp/header*.mjs`). Output to Downloads or scratch; the owner uploads to X manually.
...
- [ ] **Launch announcement card**: "The 501" card (before/after 1,432→500, or the invariant story) via the eightchain-card renderer style — REAL numbers. For the owner-approved tweet.
...
- [ ] `gh repo edit MikeyPetrillo/Agent402 --description "501 pay-per-call tools + 100 skill packs for AI agents, paid in USDC over x402 (or free via proof-of-work). CI-capped, self-hostable, MCP-native."` — and topics if any cite counts. (This is a live GitHub change — do it in Phase 5 WITH the launch, not before, so the description matches prod. Stage the exact command in the report.)
...
**Files:** `docs/ecosystem-listings.md` (copy-paste text for manual directory forms → update to 501/100), verify npm/PyPI package descriptions (swept by sync-count — confirm), `mcp/server.json` (MCP registry metadata — count derives or swept).
- [ ] Update `docs/ecosystem-listings.md` + any pending-submission copy (Cline #1849) to the new numbers/framing.
- [ ] Verify npm package.json descriptions + keywords (agent402-mcp, agent402-client, tollbooth, 8 adapters) reflect 501 (sync-count should have — confirm, fix any it missed).
...
- [ ] Post-deploy prod verification (the checklist): `/api/pricing` endpoints=501; `/health` toolCount; og card at `/og` shows 501; `/.well-known/x402` + glama.json + llms.txt + sitemap all reflect 501; `/index`+`/marketplaces` still 301; the 8-chain marketplace renders; a spot-check of 5 new tools answering live on prod (contract-source, options-chain, country-info, feed-parse, evm-rpc); PageSpeed mobile bar holds (Perf 99 / A11y 100 / BP 100 / SEO 100).
...
- [ ] Update CLAUDE.md follow-ups + memory: The 501 shipped, cap live, Phase-4/5 done.
