...
{
  "name": "Agent402",
  "description": "501 pay-per-call endpoints for AI agents over x402 — 400 tools + 100 skill packs, every one tested, priced, and settled on-chain — headless browser, live web search, OCR, PDFs, financial/SEC/macro data, durable wallet-keyed memory, and an OpenAI-compatible LLM gateway (/v1: chat, embeddings, auto-routing) — USDC on Base, Solana, Polygon, Arbitrum, Monad, Stellar & Algorand, USDG on Robinhood Chain (8 chains), or free via proof-of-work. Also an x402 Index + Smart Order Router that ranks the cheapest healthy tool across the ecosystem (auto-discovered from the CDP Bazaar). Open source, self-hostable, MCP server included.",
...
1. File an issue first (their CONTRIBUTING asks for it on significant work):
   ...
   >
   > Agent402 is an open-source x402 + MCP server with a catalog of 501
...
2. Fill the form:
   ...
   - **Short description (one line, ~150 chars):**
     ...
     ```
     501 pay-per-call web tools + skill packs for AI agents over x402 on 8 chains, or free via proof-of-work. Browser, search, OCR, finance, EDGAR, durable memory.
     ```
   ...
   - **Long description / why (if asked):**
     ...
     The catalog is 501 strong — 400 tools + 100 curated multi-tool skill packs
...
Polygon, Arbitrum, Monad, Stellar, Algorand, Robinhood Chain), so a re-crawl also refreshes any stale listing.
...
3. Verify the listing shows all eight networks (Base, Solana, Polygon,
   Arbitrum, Monad, Stellar, Algorand, Robinhood Chain) in the accepts. Also check
...
## 7. awesome-x402 (xpaysh/awesome-x402) — STALE ENTRY, needs 8-chain + The-501 update

We are already listed, but the entry predates the full 8-chain roster and the
501-strong catalog. PR a one-word-class fix:
...
2. Update the chain list to "USDC on Base, Solana, Polygon, Arbitrum, Monad,
   ...
   entry cites a tool count, set it to "501 — 400 pay-per-call tools +
   100 skill packs".
3. PR title: `Update Agent402 entry — 8-chain settlement (USDC + USDG) + 501-strong catalog`
...
2. Fill:
   ...
   - **One-liner:**
     ...
     ```
     501 strong: 400 pay-per-call web tools + 100 skill packs for AI agents over x402, every one tested and settled on-chain — USDC on Solana (and Base/Polygon/Arbitrum/Monad/Stellar/Algorand) plus USDG on Robinhood Chain — 8 chains — or free via proof-of-work. Open-source, self-hostable, MCP-native.
     ```
...
The-501 framing above (501: 400 tools + 100 skill packs, 8 chains).
