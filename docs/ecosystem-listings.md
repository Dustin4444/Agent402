...
1. File an issue first (their CONTRIBUTING asks for it on significant work):
   ...
   >
   > Agent402 is an open-source x402 + MCP server with 1,411 pay-per-call web
...
2. Fill the form:
   ...
   - **Short description (one line, ~150 chars):**
     ...
     ```
     1,411 pay-per-call web tools + 92 skill packs for AI agents over x402 (USDC on Base + 4 more chains (Solana, Polygon, Arbitrum, Stellar), or USDG on Robinhood Chain) — or free via proof-of-work. Browser, search, OCR, finance, EDGAR, durable memory.
     ```
...
2. Fill:
   ...
   - **One-liner:**
     ...
     ```
     1,411 pay-per-call web tools + 92 skill packs for AI agents over x402 — USDC on Solana (and Base/Polygon/Arbitrum/Stellar) plus USDG on Robinhood Chain — 6 chains — or free via proof-of-work. Open-source, self-hostable, MCP-native.
     ```
