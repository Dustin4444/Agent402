...
- **MCP connector.** For direct MCP access (outside Base MCP), paste `https://agent402.tools/mcp` into any MCP client. Pure-CPU tools run free there (rate-limited); wallet-only tools are payable on the connector over MPP (a paid call answers with the challenges in `_meta["org.paymentauth/payment-required"]`), or run the `agent402-mcp` npm package with a funded wallet or a prepaid card-credits key.
