...
```bash
gh repo edit MikeyPetrillo/Agent402 --description "500+ pay-per-call web tools + 100+ skill packs for AI agents - every one tested, priced, and settled on-chain. Paid in USDC over x402 on 12 chains, or free via proof-of-work on the pure-CPU tools. Self-hostable, MCP-native, deterministic. Plus the open x402 index (Find / Route / Leaderboard)."
```
