...
USDC over x402 on Base, Solana, Polygon, Arbitrum, Monad, Celo, Avalanche, Sei,
Optimism, Stellar & Algorand, plus USDG on Robinhood Chain - 12 chains. Or a few seconds of proof-of-work on the pure-CPU
...
- [ ] **Repo description** (~350-char budget; this one is 293):

```bash
gh repo edit MikeyPetrillo/Agent402 --description "500+ pay-per-call web tools + 80+ skill packs for AI agents - every one tested, priced, and settled on-chain. Paid in USDC over x402 on 12 chains, or free via proof-of-work on the pure-CPU tools. Self-hostable, MCP-native, deterministic. Plus the open x402 index (Find / Route / Leaderboard)."
```
...
gh release create v2.0.0 --title "v2.0.0 - The 500: 400 tools + 100 skill packs, every one earns its place" --notes-file /tmp/v2.0.0-notes.md
```
...
- [ ] **`[publish]`** (bump `.github/trigger-publish`) - npm + MCP Registry:
      ...
      description-only bumps: chain roster + The-500 framing).
...
- [x] ~~`wrangler deploy` in `workers/yfinance-relay/`~~ - RETIRED 2026-09-20 with the
      move to Databento: the relay and its directory are deleted and the Worker is
      gone from Cloudflare.
