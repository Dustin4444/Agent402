...
Agent402.Tools is an open-source, self-hostable x402 + MCP server with 1,419 pay-per-call tools and 92 multi-tool skill packs for AI agents. Agents pay per API call in USDC — no signup, no API keys. The wallet IS the identity.
...
- **1,419 deterministic tools** — web search, browser rendering, PDFs, OCR, finance/EDGAR data, crypto market data, DNS/security, text processing, and ~1,000 pure-CPU utilities
