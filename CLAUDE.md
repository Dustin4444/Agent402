...
- **MPP dual-stack shim (`src/mpp-shim.js`, 2026-07-23):** serves MPP (Machine
  ...
  idempotency) reads the same header it always has. Settled 200s for MPP buyers
...
- Contributor PR **#258** (agentservices.to seed) — MERGED 2026-07-13. Their listing
  ...
  follow-up PR.
- ~~CI concurrency group~~ ALREADY EXISTS (workflow-level `deploy-${{ github.ref }}`
  with cancel-in-progress in deploy.yml — verified 2026-07-23; remaining "superseded"
  noise is Railway-side and benign).
- Algorand external router shipped 2026-07-23 but DORMANT until the AVM spending wallet
  exists: Mike creates a fresh Algorand hot wallet, opts it into ASA 31566704, funds
  ~$5 USDC + ~1 ALGO for fees, sets `ALGORAND_UPSTREAM_BUYER_MNEMONIC` on Railway
  (NEVER the treasury or CI burner). No LOW-balance alarm yet — add the bucketed
  status (mirroring upstreamBuyer) when funding it.
