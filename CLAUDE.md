...
- **Heartbeat** (`heartbeat.yml`) probes prod every 15 min and opens a "production DOWN" issue on
  ...
  watches **PayAI credits** (PayAI is FIRST-TRIED for Avalanche + Sei; CDP is first for Base/Polygon/
  Arbitrum/Solana): since 2026-09-21 PayAI bills gas x 1.3 in credits ($0.001/credit; Avalanche 0.09, Sei 0.43,
  Base 2.12, Polygon 3.98, Arbitrum 6.08) with 1,000 free credits per receiving wallet for LIFE (not monthly), so
  the leg sums weighted credits since 09-21 from `/api/revenue/daily` and opens "PayAI credit allowance HIGH" at
  `PAYAI_QUOTA_WARN` (repo var, default 800). The operator chose to REGISTER (2026-09-18): `PAYAI_API_KEY_ID/SECRET`
  on Railway bill prepaid credits past the allowance (top up at merchant.payai.network); keyless, PayAI answers
  403 free_tier_exhausted and Sei goes dark. Unreadable data logs a loud warning, never a silent skip.
...
- **@solana/kit 8.3.0 + program packages on kit-8 peers, with `overrides` for @x402/svm (2026-09-18, `scripts/test-solana-kit-deps.js`):** kit 5.5.1 -> 8.3.0, `@solana-program/token` 0.9.0 -> 0.16.1, `compute-budget` 0.11.0 -> 0.18.1. @x402/svm 2.26 peers kit `>=5.1.0` but still DEPENDS on kit-5-peered program packages (token ^0.9, compute-budget ^0.11, token-2022 ^0.6.1), so a plain kit-8 install is an ERESOLVE, not a warning. Root `overrides` lift svm's three to the kit-8 releases (token 0.16.1, compute-budget 0.18.1, token-2022 0.18.0); safe because every name svm imports from them (parseTransferCheckedInstruction, getSetComputeUnitPriceInstruction, fetchMint, ...) is exported by those versions, which the pin checks textually from the svm dist so a future svm bump cannot import a missing name silently, and one kit copy is reachable from svm and from us (@coinbase/cdp-sdk keeps its own nested kit 5, unreachable). Kit 8 removed getMinimumBalanceForRentExemption / createEmptyClient / the reactive store; nothing of ours or svm's used them. The stdio `agent402-mcp` keeps `@solana/kit ^5.1.0` on purpose: published alone, its @x402/svm resolves kit 5 cleanly. Live proof owed: the paid canary's Solana leg + solana-sor-live.
...
- **agent402-openclaw is tested against a REAL OpenClaw (2026-08-26, `openclaw/test-real-install.js`, in CI):** `npm i
  ...
  gpt-4o-mini - gemini-2.5-flash left the list 2026-09-18 when the live catalog stamped it expiration_date 2026-10-20 - else cheapest fitting, else `auto` + a loud warning), metered routes advertise the METERED
...
- **STT margin cap (`src/tools/stt-kit.js`):** per-tier `maxMinutes` (4/10 since 2026-09-18; was 5/10) is enforced
  locally via a `music-metadata` duration probe BEFORE any OpenAI spend - upstream bills
  per audio minute ($0.0045 gpt-transcribe), so the cap is the margin bound, not a UX
  nicety: 4 min = 60% of the $0.03 price (5 min would be 75%, over the 70% bound). BOTH
  tiers send `gpt-transcribe` now - transcribe-pro since 2026-08-04, transcribe since
  2026-09-18 (gpt-4o-mini-transcribe, gpt-4o-transcribe and whisper-1 shut down 2027-02-26)
  - so the tiers differ only in cap and price. `STT_TIERS`/`UPSTREAM_USD_PER_MINUTE`/`STT_MARGIN`
  exported; test-stt-cap derives the bound from them. Unreadable
...
- **Broken-tool audit with PRODUCTION KEYS (2026-08-29): the 156 metered tools are outside both catalog sweeps, and one flagship was
  ...
  **Retired 2026-09-22:** contract-inspect, address-profile, token-info, token-holders and tx-inspect left the catalog and
  `blockscout-kit.js` was deleted; the routes answer 410 naming a replacement (`src/retired-tools.js`), `upstreamBuyerStatus`
  lives in `src/upstream-buyer-status.js`, token-risk reads name, supply and top holders from the keyless GoPlus record, and
  the canary's supply-chain leg, its `rail_supply-chain` status row and the `proxy` chain verb went with them.
- **A flat chat route is priced by the MODEL it is asked for (2026-09-22, `tierQuote` in llm-gateway-kit.js,
  `scripts/test-price-by-model.js` 33):** a model another flat tier serves used to be a 400 on the route it arrived at,
  which is a refusal to sell to a buyer holding the right money at the wrong door. Such a body now gets a 402 quoting
  that tier's price and, once paid, is served under that tier's caps, allowlist, failover and defaults. Applies to the
  chat, Messages, Responses and Gemini wires on nano/base/pro/premium; the answer carries `agent402_tier` naming the
  route asked for and the tier served. Mechanism: a catalog def may carry `tierQuote(body)` beside the flat `price`,
  and ONE resolver reads it (`priceFnOf` in server.js, `quoteFn` in payments.js) so the x402 accepts, the Tempo and
  Stripe challenges and the credits hold all bind the same amount; `servedTierFor` unlocks the dearer tier only when
  the settled stash covers it, so a base-priced credential on a premium body is refused AT THE PAYWALL rather than
  served. Cross-tier requests never touch the pre-paywall prompt cache (`promptCacheKey` validates against the ROUTE
  tier and throws) and are never written to it. `route-execute` passes no request, so it keeps the old 400 rather than
  dispatching a body it cannot quote. Catalog prices, `/api/pricing` and `/openapi.json` are unchanged; a priced-by-
  model route carries `pricedByModel: true` and the single copy `PRICED_BY_MODEL_NOTE`, read by both surfaces and the
  tool page. The metered tier is unaffected (its whole price is already a per-request quote).
  **`search-lite` (2026-09-22, `GET`/`POST /api/search-lite`, $0.008):** up to 5 web results (title, URL, snippet) from
  the same index `search` uses, one upstream request, no freshness filter - the cheap sample for a caller that wants a
  handful of links rather than a page. Wallet-only, in `METERED_SLUGS` and in the Brave skip set (`BRAVE_ROUTES`), so
  CI never buys a live query. The generic `serp` alias stays on `search`: a bare SERP query still resolves there on
  `/api/route` and `/api/find`.
...
- **GitHub control plane hardened (2026-08-28, applied by the operator's `gh` session):** the `agent402 / production`
  environment carried a deployment branch policy of PROTECTED BRANCHES ONLY, believed to confine the deploy and publish
  jobs to `main`. **IT DID NOT (measured 2026-09-07):** the dev-branch push run of the 0.13.1 bump created a real
  deployment to the environment from `claude/sweet-brown-i99jl3` at 01:48Z and published to npm from there, with no
  protection rule on that branch (rules API: none; the only ruleset targets `~DEFAULT_BRANCH`). The policy is now
  CUSTOM BRANCH POLICIES = `main` only (set the same day via the environments API); a dev-branch `[deploy]`/`[publish]`
  job is refused by the environment before it starts, so the marker on the merge commit (PR title) is the only path,
  and a dev push carrying `[publish]` shows a refused publish job on its run - expected, not a package failure; the
...
- **X data tools: bearer minted in Actions + a daily upstream spend cap (2026-09-06):** `x-bearer-sync.yml` (dispatch) mints the
  app-only bearer from the consumer key/secret already in Actions (OAuth 2.0 client credentials) and writes `X_BEARER_TOKEN`
  to Railway - the five x-data tools list only with it, and the deploy job carries it too when it exists as an Actions
  secret. **The X app is pay-per-use on a PREPAID balance (~$12 when enabled) that also pays for our own announce posts**, so
  x-data-kit prices every read BEFORE the call at X's card ($0.005/post, $0.010/user), refuses 503 (uncharged) once the UTC
  day's booked spend would pass `X_DATA_DAILY_MAX_USD` (default $1; `0` disables; malformed = default), and books AFTER the
  call at what X returned (X bills per item; an empty page costs nothing; same-day re-reads are deduplicated by X but charged
  by us). `xDataSpendStatus()` rides `/api/gateway-status` as `xDataSpend` (counts only). Margins are ~33-37% (priced to
  market, 08-27), not the catalog's 70%; the bet is volume on a known demand with zero cost while unlisted.
- **Equities moved off Yahoo onto a licensed feed, and three tools went with it (2026-09-20, `src/tools/databento.js`,
  `scripts/test-finance-kit.js`):** the equities tools read Yahoo's undocumented internal chart endpoint, which their
  developer terms forbid deriving income from, reached through an anti-bot crumb handshake and a Cloudflare relay routing
  around an IP block. Selling that per call was the one unlicensed redistribution in the catalog. Replaced with Databento
  (a paid commercial account, DBEQ.BASIC), which leaves redistribution scope as the open question rather than whether we
  may read the data at all. **DBEQ.BASIC is FOUR VENUES (NYSE Chicago, NYSE National, IEX, MIAX Pearl), not the
  consolidated tape** - the SIP feeds that carry whole-market volume are five figures a month to redistribute. Prices
  track the wider market; VOLUME DOES NOT, so it is named `venueVolume` on every row, `volume` is declared nowhere
  (including the MCP output schema), and the caveat rides in each answer's own `note` rather than in documentation a
  buyer may not read. `consolidate()` folds one bar per venue into one session: high/low are the extremes across venues,
  volume sums them, and OPEN/CLOSE COME FROM THE VENUE THAT TRADED THE MOST - a thin venue's print is not the day's
  close, and the pinned fixture has the thin venue printing both extremes so that cannot silently regress.
  **THREE TOOLS WERE REMOVED RATHER THAN MOVED** (`options-chain`, `premarket-quote`, `stock-dividends`) plus the
  `market-open` pack: Databento serves none of them on this subscription, and shipping a tool that answers from nowhere
  is worse than not shipping it. Catalog 613 -> 591, still above the 500 claim; `workers/yfinance-relay/` deleted.
  **Cost is bounded BEFORE each query, not discovered on the invoice** (`metadata.get_cost`, `DATABENTO_MAX_QUERY_USD`):
  a query is billed by uncompressed bytes, and a 52-week lookback priced at $0.0034 against stock-quote's $0.001 - so
  the 52-week high/low fields are GONE rather than sold at a loss, which is why that guard is worth more than the
  fields. The same guard set stock-history's advertised maximum: 365 sessions was advertised and refused from about 90
  up, so the limit is 250 (the cost guard's own 70%-of-price bound) and a test drives the advertised maximum to keep
  those two numbers from drifting apart. Three further defects the conversion surfaced, each the same shape - a
  contract nobody re-read: the calendar lookback was a flat +10 days, so a 30-SESSION ask returned 29 and the shortfall
  grew with the window (it scales now); five skill packs passed the retired `range: "1y"`/`"2y"`/`"3mo"` vocabulary,
  which the new handler ignores, so a pack promising a trailing year and computing a year-over-year return from it was
  being served 30 sessions and producing a confident wrong number (`sessionsFor()` translates, once); and validation ran
  AFTER the key check, so a malformed ticker answered 503 about our configuration instead of 400 about the input.
  CI holds no key on purpose (both slugs in `METERED_SLUGS`, live calls skipped, the unkeyed path asserted to 503
  "not configured"), and `test-leak-guard` now DECODES HTTP Basic before scanning for its canary - Databento
  base64s the credential, so the guard was reporting a clean sweep it could not actually see.
- **Hourly budget on unpaid price checks (`src/unpaid-quote-budget.js`, `UNPAID_QUOTE_BUDGET_PER_HOUR`,
  `scripts/test-unpaid-quote-budget.js` 109 in the traffic lane):** an unpaid request to a priced route earns a 402
  carrying that route's challenges, which is free to read and is where every indexer and every first-time buyer starts -
  so what this bounds is the BURST, not the practice: one client walking the whole priced catalog repeatedly learns
  nothing `/.well-known/x402`, `/openapi.json` and `/api/pricing` did not already publish, and each 402 costs a
  challenge build. Mounted in server.js AFTER the traffic classifier (a 429 is still classed) and after the Messages,
  Gemini and `/api/chain` path rewrites (an alias counts against the route it serves), and BEFORE the body parsers and
  every payment/PoW gate, so a throttled request costs no parse and no challenge build. Keyed by client ip AND the
  User-Agent product token, fixed UTC-hour windows, `Retry-After` counting down to the top of the hour, at most 20k keys
  with the least recently seen dropped first. **Never counted:** a request carrying a PLAUSIBLE payment, credits or
  proof-of-work credential; a client from its first settlement for the rest of that hour (a receipt on a `<400`
  response - every stock purchase opens with one bare unpaid request, so without this a buyer spends the budget on its
  own purchases and the 429 lands where the 402 should); named indexers, search and AI crawlers and our own
  User-Agents; `/mcp` and the connector's own loopback (marker AND a local socket); the discovery surfaces, pages and
  any unpriced route; our own signed probes. **NOT mounted under FREE_MODE** (no paywall there, and the CI sweeps boot
  in that mode). Two bypasses closed in review, both of the shape a guard invites: an exact `CATALOG` lookup on the path
  as written read `/api/uuid/`, `/API/UUID`, `//api/uuid` and `/api/%75uid` as unpriced while the paywall resolved each
  one and built its challenge, so `normalizeCatalogPath` decodes, lowercases, collapses slashes and drops a trailing one
  first (deliberately a little wider than the router: at worst a spelling that 404s is counted, and it cost a challenge
  build anyway); and any non-blank `Authorization` counted as a credential, so `Authorization: x` bought an exemption -
  `looksLikePayment` is now the ONE shape rule this and the metered quote limiter both read (server.js imports it),
  plus `looksLikePowSolution` for the `<token>:<nonce>` shape pow.js parses. Shape, never validity: the gates still
  decide. `/crawler` quotes the running figure and `/robots.txt` points at it, so the page cannot drift from the gate;
