...
- **Stellar facilitator fee account has a low-water alarm (2026-08-31, `src/stellar-facilitator-status.js`):** it pays the fee on
  EVERY Stellar settlement and was the one spending wallet we run with nothing watching it (found at 5.906 XLM, ~250 settlements
  at the observed ~0.0235 XLM per settle). The facilitator already computed it - `GET /health` returns `signerAddress`,
