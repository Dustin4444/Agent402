...
- **The 147 metered slugs are the CI blind spot, and half of it is now covered.** `METERED_SLUGS`
  ...
  believed once the harness has been shown to catch one. **STILL UNCOVERED: the synthesis half** (what the model
  returns), which needs prod keys and a few dollars - the 2026-08-29 manual-audit recipe.
...
- **Route-and-execute (`POST /api/route/execute`, $0.01, `src/tools/route-execute.js`):**
  ...
  **The spend ceiling books the WORST CASE, not the seller's declared price (2026-08-29 review):** since the price-cut
  ...
  document the number booked against the per-payer `EXTERNAL_MAX_UNSETTLED_USD` ($6) ceiling. Per-call spend was never
...
- **Stellar facilitator fee account has a low-water alarm (2026-08-31, `src/stellar-facilitator-status.js`):** it pays the fee on
  EVERY Stellar settlement and was the one spending wallet we run with nothing watching it (found at 5.906 XLM). Stellar
  settlement is CHEAP and this is a slow alarm, not an urgent one: observed `fee_charged` is 23,501 stroops = **0.00235 XLM**
  (~$0.0007 at $0.30), so 5.906 XLM is ~2,500 settlements, or ~800 if every one paid the raised inclusion bid in full. (An
  earlier draft of this entry said 0.0235 XLM and "~250 settlements" - a 10x slip, stroops are 1e7 to the XLM, and the
  dollar figure beside it was right; the alarm's default threshold was sized against the wrong number and lowered to 1 XLM
  once it was caught.) Default `FACILITATOR_LOW_BALANCE_XLM` is **1 XLM** (~425 settlements, ~136 worst case): the point is
  that an empty fee account does not look like an outage, not that 5 XLM is nearly empty - a threshold that pages with
  ...
  `xlmBalance` and `low` against `FACILITATOR_LOW_BALANCE_XLM` (default 5) - and nothing polled it, so this is a bucketed relay
...
- **MPP dual-stack shim (`src/mpp-shim.js`, 2026-07-23):** serves MPP (Machine
  ...
  **STEERED SINCE 2026-08-29 (`src/mpp-evm-domain.js` + `src/mpp-fallback.js`), because there IS a seller-side move even
  ...
  `mppEvmDomainFallback` reports counts only. The no-spoofing argument rests on `TRUST_PROXY_HOPS` staying 1 (raise it
  and `req.ip` becomes client-controlled and flags become plantable). `scripts/test-mpp-evm-domain.js` (39, in CI, boots
...
- **Coinbase Business + AgentKit (2026-08-26, from Coinbase's "get paid by AI agents" push):** (1) `agent402-tollbooth` 0.9.3: with
  ...
  Business account uses: `TOLLBOOTH_PAYTO` = the account's USDC (Base) receive address (no fee taken from the payment, CDP free tier 1,000 settlements/month then $0.001; NOT Bazaar-listed - the CLI's wildcard route carries no discovery extension). The key is validated at boot by minting one JWT.
...
- **mppx 0.9.2 (2026-09-02):** taken after a read of the 0.9.0-0.9.2 changelog: 0.9.0 removes machineUSD (never configured
  ...
  (`MCP_PAYMENT_VERIFICATION_FAILED_CODE`, mcp-mpp.js; mcp-http.js sends it whenever a `problem` rides along; the 0.9 client
