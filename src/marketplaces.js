...
function pillar({ tag, title, body, href, cta, stat }) {
  return `<a href="${esc(href)}" style="display:flex;flex-direction:column;gap:10px;background:var(--surface);color:var(--on-dark);border:1.5px solid var(--ink);padding:22px 22px 20px;text-decoration:none;">
...
export function marketplacesPage(baseUrl, { chainSellerCounts = {}, indexSnapshot, leaderboardSnap, toolCount = 0, settlements = 0 } = {}) {
  ...
  const chainCard = ([slug, name]) => {
    ...
    return `<a href="/${slug}" style="display:flex;align-items:center;gap:12px;border:1.5px solid var(--ink);background:var(--card);padding:14px 16px;text-decoration:none;color:var(--ink);">
  ...
  const body = `
  <!-- HERO -->
  <header style="border-bottom:1.5px solid var(--ink);background:var(--paper);">
  ...
  <section style="max-width:1180px;margin:0 auto;padding:56px 30px 0;">
    ...
    ${board.length ? `<div style="border:1.5px solid var(--ink);background:var(--card);padding:16px 20px;margin-top:12px;">
  ...
  <section style="max-width:1180px;margin:0 auto;padding:56px 30px 70px;">
    <div class="mkts-onramps" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div style="border:1.5px solid var(--ink);background:var(--paper);padding:26px 24px;">
