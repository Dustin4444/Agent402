...
export function ledgerHomePage(baseUrl, catalog, stats, leaderboardSnapshot, skillPacks) {
  ...
  const websiteLd = { "@type": "WebSite", "@id": `${baseUrl}/#website`, name: "Agent402.Tools", alternateName: "Agent402 - Agentic Finance (AIFI) applied layer", url: baseUrl, publisher: { "@id": `${baseUrl}/#organization` }, description: "The applied layer of Agentic Finance (AIFI): open index, Smart Order Router and on-chain ranking for agents paying and getting paid over x402 and MPP.", about: { "@type": "DefinedTerm", name: "Agentic Finance", alternateName: "AIFI", url: `${baseUrl}/agentic-finance` }, potentialAction: { "@type": "SearchAction", target: `${baseUrl}/api/find?q={search_term_string}`, "query-input": "required name=search_term_string" } };
  const appLd = { "@type": "SoftwareApplication", "@id": `${baseUrl}/#app`, name: "Agent402", url: baseUrl, applicationCategory: "DeveloperApplication", operatingSystem: RAILS.map((r) => r.name).join(", "), license: "https://www.gnu.org/licenses/agpl-3.0.html", description: `Open-source, self-hostable Agentic Finance (AIFI) server for x402 + MPP: ${fmtNum(count)} deterministic pay-per-call tools and ${packCount}+ skill packs for AI agents, plus an open index, Smart Order Router and on-chain seller leaderboard.`, offers: { "@type": "AggregateOffer", offerCount: String(count), lowPrice: "0.001", highPrice: "1.50", priceCurrency: "USD", description: "Per-call micropayments in USDC on eleven chains plus USDG on Robinhood Chain, or free with proof-of-work" } };
  ...
  const faqs = [
    { q: "What is Agentic Finance (AIFI)?", a: "Agentic Finance (AIFI) is software agents transacting on their own: discovering a service, paying per request from a non-custodial wallet over open protocols such as x402 and MPP, receiving a verifiable receipt, and earning per request in return. Agent402 is its applied layer: the tools agents buy, the index and Smart Order Router that find and pay the best seller, the tollbooth that lets any site earn from agents, and on-chain transparency for all of it. Full explainer at /agentic-finance." },
  ...
  <div style="max-width:1180px;margin:0 auto;padding:52px 30px 0;position:relative;">
    <div class="hm-hero" style="display:grid;grid-template-columns:1.06fr .94fr;gap:56px;align-items:start;">
      <div>
        ...
        <h1 style="font-weight:800;font-size:66px;line-height:.92;letter-spacing:-.038em;margin:0 0 22px;color:var(--ink);">The applied layer of <span style="color:var(--accent);">Agentic Finance</span>.</h1>
        <p style="font-size:18.5px;line-height:1.5;color:var(--muted);max-width:560px;margin:0 0 30px;"><a href="/agentic-finance" style="color:var(--ink);text-decoration:none;border-bottom:1px solid var(--accent);">Agentic Finance (AIFI)</a> is agents paying and getting paid on their own, over <a href="/what-is-x402" style="color:var(--ink);text-decoration:none;border-bottom:1px solid var(--accent);">x402</a> and <a href="/what-is-mpp" style="color:var(--ink);text-decoration:none;border-bottom:1px solid var(--accent);">MPP</a>. Most of the ecosystem ships the protocol. Agent402 ships the <strong style="color:var(--ink);font-weight:700;">market that runs on it</strong> - an open index, a neutral router, and an on-chain ranking of every seller. List your API and get paid in USDC per call, straight to your wallet. No signup, no API keys, and nothing deducted from your price.</p>
