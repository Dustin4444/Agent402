...
export function makeDossierHandler(tierSlug) {
  ...
  return async (input) => {
    ...
    // 1) STRUCTURED DATA - fan out to EDGAR + quote in parallel (the moat).
    const [quote, k10, q10, k8, insider] = await Promise.all([
    ...
    const webSourceLines = numbered.filter((s) => !s.title.includes("SEC EDGAR")).map((s) => `[${s.n}] ${s.title} - ${s.url}`).join("\n");
...
3. CITATIONS: cite every substantive claim inline with [n] matching the numbered SOURCES (SEC filings AND web sources are numbered - cite them). A citation is ONLY a bracketed number, e.g. [14] or [3][7] - NEVER put words, notes, ranges, or explanations inside the brackets (not "[14 for the release]", not "[13-adjacent]"), and never use a word-tag like [research]/[web]/[data] or cite "the material". If a source only partly supports a point, say so in the sentence, not inside the bracket. The LIVE QUOTE and FORM 4 INSIDER data are given directly, not as numbered sources: reference them in prose ("the live quote shows...", "Form 4 filings in the window show...") WITHOUT a bracket. Only attach [n] to a claim its source supports.
...
=== WEB SOURCES (numbered) ===\n${webSourceLines || "(none)"}`;
    ...
    const out = format === "json"
      ...
      : { dossier, company, ticker, sources: numbered, meta };
    if (process.env.RESEARCH_DEBUG === "1") out._debug = { webAnswers: webGood.map((r) => ({ q: r.q, answer: r.answer })), quoteBlock, insiderBlock };
    return out;
  };
}
