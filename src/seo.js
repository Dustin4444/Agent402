...
3b. **Non-custodial payment tooling.** /api/x402-quote, /api/x402-verify, /api/usdc-balance, /api/tx-status, /api/gas-estimate, /api/ens-resolve, and /api/transfer-authorization help an agent pay over x402 with its OWN key across Base, Polygon, Arbitrum, Optimism, Ethereum, and Robinhood Chain — decode a 402 quote, build the EIP-3009 authorization to sign, and verify the settlement on-chain. Agent402 never holds or moves funds.
...
- \`GET /api/find?q={task}\` — resolve a task description to the best-matching tools (route, price, input schema, ready example) in one call; skips the token-heavy "search to find a tool" step. Also accepts \`POST {"task":"..."}\`. Weak/empty matches carry a \`hint\` pointing at \`/api/wish\`.
- \`POST /api/wish {"need":"...", "context":"..."}\` — nothing here fit? Say what you needed in plain text (free, no wallet, lightly rate-limited). Requests are clustered by normalized text; repeated demand for the same thing gets built. Also exposed as the MCP tool \`request_tool\` on \`/mcp\`. Aggregate view (normalized text, counts, no raw context) at \`GET /api/wishes\`.
