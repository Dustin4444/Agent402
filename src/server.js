...
const _premiumHandlers = Object.fromEntries([...RESEARCH_DEEP_TOOLS, ...DOSSIER_TOOLS, ...FUND_TOOLS, ...DOMAIN_AUDIT_TOOLS, ...RECALL_TOOLS, ...IPO_TOOLS].map((t) => [t.slug, t.handler]));
const _humanGenerate = async (kind, slug, input, ctx = {}) => {
    const h = _premiumHandlers[slug];
    if (!h) throw new Error("no handler for " + slug);
    const argOf = { dossier: (v) => ({ ticker: v }), fund: (v) => ({ manager: v }), domain: (v) => ({ domain: v }), research: (v) => ({ query: v }), recall: (v) => ({ query: v }), ipo: (v) => ({ days: 7, keyword: v }) };
    ...
    const titleOf = { dossier: () => (out?.company ? `${out.company} (${out.ticker})` : fallbackTitle), fund: () => out?.manager || fallbackTitle, domain: () => out?.domain || fallbackTitle, recall: () => (out?.query ? `FDA recalls: ${out.query}` : fallbackTitle), ipo: () => out?.title || "IPO pipeline" };
    return {
      report,
      kind,
