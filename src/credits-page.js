...
export function creditsPage(baseUrl) {
  const cards = Object.entries(CREDIT_PACKS).map(([key, p]) => `
    <div class="pcard" data-pack="${esc(key)}" style="text-align:center;">
      ...
      <p style="margin:4px 0 16px;">${esc(p.cents === 2000 ? "about 20,000 calls at the $0.001 floor, or four $5 reports" : p.cents === 5000 ? "try the whole catalog; research, dossiers, audits included" : "for a team or an agent fleet; never expires")}</p>
