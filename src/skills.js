...
export const PACK_PRICES = {
  ...
  "earnings-deep-dive": 0.033, // 4 tools, parts $0.036
  "options-analytics": 0.017, // 4 tools, parts $0.018
  "fixed-income-desk": 0.017, // 5 tools, parts $0.018
  "defi-protocol-scanner": 0.025, // 4 tools, parts $0.027
  ...
  "macro-economics": 0.032, // 7 tools, parts $0.035
  "macro-dashboard": 0.065, // 14 tools, parts $0.072
  "dns-network-ops": 0.011, // 6 tools, parts $0.012
  "crypto-research": 0.054, // 7 tools, parts $0.06
  "content-extraction": 0.035, // 6 tools, parts $0.038
  ...
  "macro-context": 0.036, // 8 tools, parts $0.04
  "regulatory-watch": 0.023, // 5 tools, parts $0.025
  "search-and-cite": 0.101, // 5 tools, parts $0.112
  ...
  "cheapest-rail": 0.009, // 4 tools, parts $0.009
  "company-dossier": 0.037, // 5 tools, parts $0.041
  "domain-intel": 0.016, // 6 tools, parts $0.017
  "crypto-dossier": 0.041, // 6 tools, parts $0.045
  "earnings-watch": 0.015, // 3 tools, parts $0.016
  "insider-alert": 0.01, // 3 tools, parts $0.011
  "ipo-watch": 0.023, // 3 tools, parts $0.025
  "yield-dashboard": 0.014, // 3 tools, parts $0.015
  "inflation-check": 0.018, // 4 tools, parts $0.02
  ...
  "entity-enrich": 0.022, // 6 tools, parts $0.024
...
export function skillPackPage(baseUrl, slug, catalog) {
  ...
  // How the price is set, from the parts the served catalog prices right now:
  // the pack is the sum of its tools less the bundle discount (PACK_PRICES is
  // generated by scripts/pack-prices.js from the same rule).
  const usd = (p) => Number(String(p ?? "").replace(/[^0-9.]/g, "")) || 0;
  const members = [...new Set(pack.toolSlugs)].map((s) => ix.get(s)).filter(Boolean);
  const partsUsd = members.reduce((a, t) => a + usd(t.price), 0);
  const priceNote = packTool && members.length
    ? `Bought one at a time, these ${members.length} tools cost $${fmtPackUsd(Math.round(partsUsd * 1000) / 1000)} together; the pack is that sum less a 10% bundle discount, rounded up to the $0.001 settlement floor, which is ${e(packTool.price)}.`
    : "";

  const cell = (s) => `<td style="border:1px solid var(--hairline);padding:10px 12px;vertical-align:top;">${s}</td>`;
  const argRows = (pack.promptArgs || []).map((a) =>
    `<tr>${cell(`<code>${e(a.name)}</code>`)}${cell(a.required ? "yes" : "no")}${cell(e(a.description || ""))}${cell(a.substitute != null ? `<code>${e(String(a.substitute))}</code>` : "")}</tr>`).join("");

  // What one call returns: the envelope keys from the pack route's own example.
  const packExample = packTool?.discovery?.output?.example;
  const exampleSteps = Array.isArray(packExample?.steps) ? packExample.steps : [];
  const returnsHtml = packExample && typeof packExample === "object"
    ? `<h2 style="font-weight:800;font-size:22px;margin-top:40px;letter-spacing:-.01em;">What one call returns</h2>
<p style="color:var(--muted);font-size:15px;line-height:1.7;">A JSON object with ${Object.keys(packExample).map((k) => `<code>${e(k)}</code>`).join(", ")}${exampleSteps.length ? `; <code>steps</code> holds one entry per tool (${exampleSteps.slice(0, 8).map((s) => e(s.tool || s.slug || s.name || "")).filter(Boolean).join(", ")}${exampleSteps.length > 8 ? ", ..." : ""}), each with its own result or error` : ""}. Full example on the <a href="/tools/skill-${e(pack.slug)}" style="color:var(--accent);">API page</a>.</p>`
    : "";
...
${priceNote ? `<p style="color:var(--muted);font-size:14.5px;line-height:1.7;margin:14px 0 0;">${priceNote}</p>` : ""}

<h2 style="font-weight:800;font-size:22px;margin-top:40px;letter-spacing:-.01em;">Workflow</h2>
<ol class="sk-wf">${steps}</ol>

${argRows ? `
<h2 style="font-weight:800;font-size:22px;margin-top:40px;letter-spacing:-.01em;">Arguments</h2>
<div style="overflow-x:auto;"><table style="border-collapse:collapse;width:100%;font-size:14px;"><tr><th style="border:1px solid var(--hairline);padding:10px 12px;text-align:left;background:var(--card);">Name</th><th style="border:1px solid var(--hairline);padding:10px 12px;text-align:left;background:var(--card);">Required</th><th style="border:1px solid var(--hairline);padding:10px 12px;text-align:left;background:var(--card);">Description</th><th style="border:1px solid var(--hairline);padding:10px 12px;text-align:left;background:var(--card);">Example</th></tr>${argRows}</table></div>` : ""}
