# Agent402.Tools — the open x402 index (Find · Route · Leaderboard) + 1,425 tools & 92 skill packs for AI agents
...
>
> Plus the whole **1,425-tool catalog** and **92 curated skill packs** (multi-tool
...
**Give your AI agent 1,425 ready-to-use web tools — plus 92 multi-tool skill packs — from one server. Browser
...
**3. Clone and host the whole thing** (all 1,425 tools as an HTTP API + MCP, free mode, no payments):
...
## What's in the catalog (1,425 tools)
...
**OpenClaw agents:** drop in [`skills/openclaw/agent402/SKILL.md`](skills/openclaw/agent402/SKILL.md) — teaches an OpenClaw agent to find, pay (x402 USDC on Base, six other chains accepted, or free proof-of-work), and call any of the 1,425 tools.
...
- **Everything is tested** — CI calls all 1,425 tools with their own documented
...
a ready-to-run **x402 server** that already speaks the protocol and ships 1,425
...
**Works with [AWS Bedrock AgentCore Payments](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/payments.html) out of the box** — AgentCore orchestrates x402, which is the protocol Agent402 already speaks. Point the AgentCore Gateway at `https://agent402.tools/mcp` for all 1,425 tools, or use [`agent402-strands`](https://www.npmjs.com/package/agent402-strands) for a curated subset inside a [Strands](https://strandsagents.com) agent. Five-minute recipe: [wiki: AWS Bedrock AgentCore](https://github.com/MikeyPetrillo/Agent402/wiki/AWS-Bedrock-AgentCore).
