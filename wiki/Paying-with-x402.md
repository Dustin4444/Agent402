...
The payer needs **only USDC on Base, Solana, Polygon, Arbitrum, Monad, Celo, Avalanche, Sei, X Layer, Stellar, or Algorand** — no ETH, no account, no API key. (Sellers can also settle **USDG on Robinhood Chain** when the operator enables it.)
