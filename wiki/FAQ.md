...
**What does it cost?**
Flat per-call prices starting at **$0.001**. Most tools are $0.001–$0.02; premium inference, media, and multi-tool skill packs are priced higher (up to $1.50 for the biggest pack). Every price is published in [`/api/pricing`](https://agent402.tools/api/pricing) and quoted exactly in every 402 response. Report products (`/v1/research`, `/v1/dossier`, `/v1/fund`, `/v1/domain-audit`, `/v1/recall-report`, `/v1/insider-report`, `/v1/token-risk`) are priced per finished report, $3 to $19; monitors at [`/monitors`](https://agent402.tools/monitors) are the one subscription, $5 per month per target.
