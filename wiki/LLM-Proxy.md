...
> **Looking for the OpenAI-compatible wire paths?** The [[LLM Gateway (OpenAI /v1)|LLM-Gateway]] serves standard `chat/completions`, `embeddings`, `images/generations` and `audio/speech` at `https://agent402.tools/v1` - eight paid endpoints from $0.002, model-optional auto-routing, streaming, and response caching. Any OpenAI SDK adopts it by changing `base_url`. The `/api/llm*` tools below remain available as the simpler custom-JSON proxy.
