...
- 1,425 deterministic, pay-per-call tools + 92 multi-tool skill packs from Agent402, callable from an AgentCore-hosted agent
...
## Option 1: Gateway target (zero code, all 1,425 tools)
...
3. **Done.** Your agent sees the four Agent402 MCP tools (`search_tools`, `find_tool`, `call_tool`, `about_agent402`) plus, via `call_tool`, the full 1,425-tool catalog. Payments — proof-of-work for free tools, USDC for wallet-only — happen in the request path; AgentCore logs every settled call to CloudWatch.
