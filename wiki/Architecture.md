...
```
   ...
   agent (buyer) ───▶ │  Express                                     │
                      ...
                      │   1,413 tool handlers (pure fns + kits)      │
                      └──────┬───────────────┬───────────────────────┘
                             │               │
...
- **Payments** (`src/payments.js`): `@x402/express` middleware quoting USDC on Base, Solana, Polygon, Arbitrum & Stellar, settled through the **Coinbase CDP facilitator** (`CDP_API_KEY_ID/SECRET`; `FACILITATOR_URL` overrides). Multi-chain USDC schemes are registered in code. Robinhood Chain (USDG / Global Dollar, chain id 4663) settles through an operator-supplied facilitator (`ROBINHOOD_FACILITATOR_URL`) with a custom USDG money parser.
