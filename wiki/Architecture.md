...
- **Settlement evidence comes from the chain, never from a registry.** Before the router pays an outside seller from our wallet, it wants evidence that others already pay them. That evidence is read from the chain on every rail we can spend on: Base and Solana from their own USDC transfers, Tempo from its own, Algorand from inbound USDC-ASA transfers to the payTo the seller advertises. Third-party registries (Coinbase's Bazaar, a facilitator's catalog) are a source of CANDIDATES and never the only source: until 2026-09-18 a seller we had indexed ourselves could settle any volume on chain and still read as unproven simply because it was absent from someone else's list, which is both wrong and the opposite of what we say about ranking neutrally. An unreadable chain is a refusal, not evidence, and evidence a seller inherits from a wallet it does not itself pay to is refused outright.
