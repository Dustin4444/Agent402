...
// eleven rails has a mark, the marks are single-tone (currentColor, no baked
...
const CHAINS = ["base", "solana", "polygon", "arbitrum", "monad", "celo", "avalanche", "sei", "stellar", "algorand", "robinhood"];

// --- every rail has a mark, and it renders to an <svg> ---------------------------
ok(CHAIN_ORDER.length === 11, `eleven rails in CHAIN_ORDER (got ${CHAIN_ORDER.length})`);
...
ok((strip.match(/<svg/g) || []).length === 11, "strip renders exactly eleven marks");
