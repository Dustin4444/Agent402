...
// ten rails has a mark, the marks are single-tone (currentColor, no baked
...
const CHAINS = ["base", "solana", "polygon", "arbitrum", "monad", "celo", "avalanche", "stellar", "algorand", "robinhood"];

// --- every rail has a mark, and it renders to an <svg> ---------------------------
ok(CHAIN_ORDER.length === 10, `ten rails in CHAIN_ORDER (got ${CHAIN_ORDER.length})`);
...
ok((strip.match(/<svg/g) || []).length === 10, "strip renders exactly ten marks");
