// Announcement demo card for a PAID treasury-yield-curve call — renders a
...
// The inset rows read the capture's real FRED DGS* curve; the settle line
// carries the real tx. Real numbers only; --preview tags a fixture layout.
//
// Usage:
//   node scripts/fred-demo-card.js --from paid-demo-result.json --out card.png [--preview]
...
const FROM = arg("--from");
const OUT = arg("--out") || "fred-demo-card.png";
const PREVIEW = args.includes("--preview");
if (!FROM) {
  console.error("usage: node scripts/fred-demo-card.js --from <paid-demo-result.json> --out <png> [--preview]");
  process.exit(1);
}
...
function cardSvg(cap) {
  ...
  const pt = (label, v) =>
    `<tspan fill="${B.muted}">${label} </tspan><tspan font-weight="700" fill="${B.text}">${v == null ? "n/a" : Number(v).toFixed(2)}</tspan>`;
  ...
  <text x="96" y="130" font-size="22" font-family=${mono}><tspan font-weight="700" fill="${B.text}">Agent402 /api/treasury-yield-curve</tspan><tspan fill="${B.muted}"> · FRED DGS*, paid per call · ${esc(liveDate)}</tspan></text>
  ${okRow(180, "tool", "11 maturities, deterministic", `$${cap.quote?.usd} per call`)}
  ...
  ${okRow(282, "source", "FRED DGS series, St. Louis Fed", "primary source, no key")}
  ...
  <text x="126" y="410" font-size="19" font-family=${mono}><tspan font-weight="700" fill="${B.text}">US Treasury curve</tspan><tspan fill="${B.muted}"> · record date ${esc(r.recordDate)} · percent</tspan></text>
  <text x="126" y="440" font-size="19" font-family=${mono}>${pt("1M", r.mo1)}<tspan fill="${B.muted}"> · </tspan>${pt("3M", r.mo3)}<tspan fill="${B.muted}"> · </tspan>${pt("1Y", r.yr1)}<tspan fill="${B.muted}"> · </tspan>${pt("2Y", r.yr2)}<tspan fill="${B.muted}"> · </tspan>${pt("5Y", r.yr5)}<tspan fill="${B.muted}"> · </tspan>${pt("10Y", r.yr10)}<tspan fill="${B.muted}"> · </tspan>${pt("30Y", r.yr30)}</text>
  ${insetNote}
...
try {
  ...
  console.error(`fred-demo-card: ${e?.message || e}`);
  process.exit(2);
}
