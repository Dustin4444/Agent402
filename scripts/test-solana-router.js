...
// ---- 4. proven-seller gate (fail closed) --------------------------------
{
  ...
  const below = await assertProvenSolanaSeller(payTo, { inboundFn: async () => 1, minCount: 3 }).then(() => null, (e) => e);
  ok(below && below.statusCode === 409 && /floor 3/.test(below.message), "a seller under the distinct-payer floor is refused 409 with the floor named");
  ok((await assertProvenSolanaSeller(payTo, { inboundFn: async () => 5, minCount: 3 })) === 5, "a seller at/over the distinct-payer floor passes and the count is returned");
...
// ---- 6. resolve-time gate: skip, never abort --------------------------
{
  ...
  const thin = await passesSolanaResolveGate({ header: mk([solAccept]), inboundFn: async () => 1, minCount: 3 });
  ok(thin.ok === false && /floor 3/.test(thin.reason), "an unproven candidate is SKIPPED with the count in the reason - never an abort");
