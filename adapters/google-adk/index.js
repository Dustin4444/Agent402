// agent402-google-adk — turn Agent402 into Google ADK FunctionTool instances
// the agent can pick up directly. Four meta tools, all free to discover:
//
...
//
// Why four and not "one tool per slug"? Frameworks balk at registering 1,420
...
//     tools,
//     instruction: "You have access to Agent402's 1,420 web tools. Use agent402_find to discover the right tool, then agent402_call to invoke it.",
//   });
//
...
// the `@google/adk` dep, or if you're wrapping the tools yourself.
//
// Implementation note: we inline the proof-of-work solver + call orchestration
// instead of depending on agent402-client at runtime, so this package
...
function solvePow({ challenge, difficulty, token }) {
  let nonce = 0;
  while (leadingZeroBits(createHash("sha256").update(`${challenge}:${nonce}`).digest()) < difficulty) nonce++;
  return `${token}:${nonce}`;
}
...
async function callTool({ base, slug, params, payFetch, fetchImpl }) {
  ...
  const idem = `a402adk-${createHash("sha256").update(`${slug}:${JSON.stringify(params)}:${Date.now()}:${Math.random()}`).digest("hex").slice(0, 24)}`;
...
/**
 * Framework-native Google ADK tools. Dynamically imports `@google/adk` and
 * `zod` (both peer dependencies) so the spec path works without them.
 *
 * Returns an array of FunctionTool — pass into `new LlmAgent({ tools })`.
 */
export async function agent402Tools(opts) {
  const [{ FunctionTool }, { z }] = await Promise.all([
    import("@google/adk"),
    import("zod"),
  ]);
...
// Minimal JSON-Schema to Zod converter for the shapes we emit. Not a general
